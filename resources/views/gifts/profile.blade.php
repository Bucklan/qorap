...
@section('content')
            ...
            <div class="row">
                <div class="col-md-4 aside aside--left">
                    <div class="list-group">
                        <a href="{{url('/profile')}}" class="list-group-item active">{{__('footer.myaccount')}}</a>
                        <a href="#" class="list-group-item">{{__('profile.My Addresses')}}</a>
                        <a href="{{url('/selected')}}" class="list-group-item">{{__('footer.mywishlist')}}</a>
                        <a href="{{url('/cart')}}" class="list-group-item">{{__('footer.viewcart')}}</a>
                        <a href="#" class="list-group-item">{{__('footer.orderhistory')}}</a>
                    </div>
                </div>
                <div class="col-md-14 aside">
                    <h1 class="mb-3">{{__('footer.myaccount')}}</h1>
                    <div class="row vert-margin">
                        <div class="col-sm-9">
                            <div class="card">
                                <div class="card-body">
                                    <h3>{{__('profile.Personal Info')}}</h3>
                                    <p><b>{{__('register.fullname')}}</b> {{$user->name}}<br>
                                        <b>{{__('login.email')}}</b> {{$user->email}}<br>
                                        <b>{{__('profile.photo')}}</b> <img src="{{asset($user->image)}}" alt=""></p>
                                    <div class="mt-2 clearfix">
                                        <a href="#" class="link-icn js-show-form" data-form="#updateDetails"><i
                                                class="icon-pencil"></i>{{__('buttons.edit')}}</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <form action="{{route('update',$user->id)}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="card mt-3 d-none" id="updateDetails">
                            <div class="card-body">
                                <h3>{{__('profile.Update Account Details')}}</h3>
                                <div class="row mt-2">
                                    <div class="col-12">
                                        <label class="text-uppercase">{{__('register.fullname')}}</label>
                                ...
                                <div class="row mt-2">
                                    <div class="col-12">
                                        <label class="text-uppercase">{{__('login.email')}}</label>
                                        <div class="form-group">
                                            <input type="email" name="email" readonly class="form-control form-control--sm @error('email') is-invalid @enderror"
                                                   value="{{$user->email}}">
                                            @error('email')
                                            <div class="invalid-feedback">{{$message}}</div>
                                            @enderror
                                    ...
                                    <div class="col-12">
                                        <label class="text-uppercase">{{__('profile.photo')}}:</label>
                                        <div class="form-group">
                                            <input class="form-control @error('image') is-invalid @enderror" value="{{$user->image}}" id="image" type="file"
                                ...
                                <div class="mt-2">
                                    ...
                                    <button class="btn ml-1">{{__('buttons.edit')}}</button>
                                </div>
                            </div>
                        </div>
                    </form>
    ...
    {{--                        <img alt="Midone - HTML Admin Template" class="rounded-full" src="{{asset($user->image)}}">--}}
    {{--                    </div>--}}
    {{--                    <div class="ml-5">--}}
